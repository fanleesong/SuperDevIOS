...
void DrawGridWater()
{
  ...
  float max = gRenderDensityMax;
	...
	float *vts = (float *)calloc(nx*ny*4*3, sizeof(float));
  int i, j,n=0;
  float grey;
  for (i = nx-1 ; i-- != 0 ; )
  {
    for (j = ny-1 ; j-- != 0 ; )
    {
        ...
        glColor4f(0, 0, grey,1.0f);
		vts[n++]=x;vts[n++]=y;vts[n++]=1;
        grey = (densities(i+1, j) - min) / (max - min);
        glColor4f(0, 0, grey,1.0f);
		vts[n++]=x+dx;vts[n++]=y;vts[n++]=1;
        grey = (densities(i+1, j+1) - min) / (max - min);
        glColor4f(0, 0, grey,1.0f);
		vts[n++]=x+dx;vts[n++]=y+dy;vts[n++]=1;
        grey = (densities(i, j+1) - min) / (max - min);
        glColor4f(0, 0, grey,1.0f);
		vts[n++]=x;vts[n++]=y+dy;vts[n++]=1;
    }
  }
	glVertexPointer(3,GL_FLOAT,0, vts);
	glDrawArrays(GL_POINTS, 0, nx*ny*4);
...
void ReadConfig()
{
	...
	gWindowW = 320;
	gWindowH = 480;
	gRenderParticles = true;
	gNRenderSamplesX = 0;
	gNRenderSamplesY = 0;
	gRenderDensityMin = 100.0f;
	gRenderDensityMax = 500.0f;
